...
function cell(value: unknown, col: Column, width: number) {
  const text = col.format ? col.format(value) : String(value ?? "")
  return col.align === "right" ? text.padStart(width) : text.padEnd(width)
}

function colwidth(col: Column, data: Record<string, unknown>[]) {
  if (col.width) return col.width
  return Math.max(
    col.label.length,
    ...data.map((row) => (col.format ? col.format(row[col.key]) : String(row[col.key] ?? "")).length),
  )
}
  ...
  const header = createMemo(() =>
    props.columns
      .map((col, index) => {
        ...
        return col.align === "right" ? label.padStart(widths()[index]) : label.padEnd(widths()[index])
      })
      .join("  "),
  )
    ...
    <box>
      ...
      <For each={props.data}>
        {(row, index) => {
          ...
          const open = () => index() === props.expanded
