...
Database tools for inspecting and managing the OpenCode SQLite database.

```bash
opencode db [query]
```

When run without arguments, opens an interactive `sqlite3` shell. When given a SQL query, runs it and prints the results.
...
# Run a query
opencode db "SELECT id, title FROM session ORDER BY created_at DESC LIMIT 10"
```
...
Migrate legacy JSON data to SQLite (merges with existing data).
